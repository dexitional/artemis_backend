...
module.exports = {
  ...
  fetchStudentSlipAIS: async (req, res) => {
    ...
    try {
      ...
      if (slip && slip.length > 0) {
        res.status(200).json({
          success: true,
          data: { regdata: slip, user: user && user[0] },
        });
      } else {
        res.status(200).json({ success: true, data: [], msg: "No records!" });
      }
  ...
  fetchStudentReg: async (req, res) => {
    ...
    try {
      var student = await Student.fetchStudentProfile(refno);
      if (student && student.length > 0) {
        const { prog_id, semester, major_id, indexno, session_semester } =
          student[0];
        ...
        if (mt.length > 0) {
          ...
          if (mj)
            courses.meta = {
              min_credit: mj.min_credit,
              max_credit: mj.max_credit,
              max_elective: mj.max_elective,
    ...
    try {
      ...
      if (indexno) {
        ...
        const dm = {
          ...
          credits_count: courses.reduce(
            (acc, val) => acc + parseInt(val.credit),
            0
          ),
          meta_dump: JSON.stringify(courses),
        };
