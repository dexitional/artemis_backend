...
Router.post("/ais/backlog/courseadd", SSOController.processBacklogCourseAdd);
Router.post("/ais/backlog/coursedel", SSOController.processBacklogCourseDel);
...
// SERVICE LETTERS
Router.get("/ais/letters", SSOController.fetchServiceLetters);
Router.post("/ais/letters", SSOController.postServiceLetter);
Router.delete("/ais/letters/:id", SSOController.deleteServiceLetter);
// STREAMS
...
// TRANSWIFT routes
Router.get("/ais/transwift", SSOController.fetchTranswifts);
Router.get("/ais/transwift/:id", SSOController.fetchTranswift);
...
Router.get("/fixfinance", async (req, res) => {
  ...
  const ss = await db.query("select s.refno as sid,t.id as tid from fms.transaction t left join fms.studtrans m on t.id = m.tid left join ais.student s on t.refno in (s.refno,s.indexno) where t.transtype_id in (2,4) and t.refno is not null and m.tid is null");
  var count = 0;
  if (ss.length > 0) {
    for (var s of ss) {
      const st = 
